...
export const setupDocument = async (context: PDFGenerationContext): Promise<{ doc: jsPDF; logoYOffset: number }> => {
  ...
  // Add the large "CALIFORNIA | TELECOM" header at the top - bold and dark blue
  doc.setFontSize(28);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(0, 0, 139); // Dark blue color
  doc.text('CALIFORNIA | TELECOM', 20, 30);
  
  // Document type "Agreement" in top right - gray color, normal weight
  doc.setFontSize(28);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(128, 128, 128); // Gray color
  doc.text('Agreement', 155, 30);
...
export const addAgreementDetailsBox = (doc: jsPDF, context: PDFGenerationContext): void => {
  ...
  // Box content with proper spacing and alignment
  doc.setFontSize(9);
  doc.setFont('helvetica', 'bold');
  ...
  doc.text('Date:', boxX + 3, boxY + 16);
  doc.text('Expires', boxX + 3, boxY + 24);
  doc.text('Account', boxX + 3, boxY + 32);
  doc.text('Manager', boxX + 3, boxY + 39);
  ...
  doc.text(quoteNumber, boxX + boxWidth - 3 - numberWidth, boxY + 8);
  doc.text(dateText, boxX + boxWidth - 3 - dateWidth, boxY + 16);
  doc.text(expiresText, boxX + boxWidth - 3 - expiresWidth, boxY + 24);
  doc.text(accountManager, boxX + boxWidth - 3 - managerWidth, boxY + 35);
};
