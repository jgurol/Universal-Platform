...
export const generateQuotePDFWithPDFShift = async (quote: Quote, clientInfo?: ClientInfo): Promise<Blob> => {
  console.log('[PDFShift] Generating PDF with PDFShift API');
  
  const apiKey = import.meta.env.VITE_PDFSHIFT_API_KEY;
  
  if (!apiKey) {
    throw new Error('PDFShift API key not configured. Please add VITE_PDFSHIFT_API_KEY to your environment variables.');
  }
  ...
  try {
    ...
    console.error('[PDFShift] Error generating PDF:', error);
    throw error;
  }
};
